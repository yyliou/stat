...
# Chapter 6: The Normal Distribution {background-color="#2c4a6e"}
...
```{r normal-shape, echo=T, warning=FALSE}
...
```{r ex6-1-plot, echo=T, warning=FALSE}
x <- seq(-4, 4, length.out = 300)
df <- data.frame(x = x, y = dnorm(x))
ggplot(df, aes(x, y)) +
...
```{r ex6-2-plot, echo=T, warning=FALSE}
x <- seq(-4, 4, length.out = 300)
df <- data.frame(x = x, y = dnorm(x))
ggplot(df, aes(x, y)) +
  geom_line(color = "#2c4a6e", linewidth = 1) +
...
```{r ex6-3-plot, echo=T, warning=FALSE}
x <- seq(-4, 4, length.out = 300)
df <- data.frame(x = x, y = dnorm(x))
ggplot(df, aes(x, y)) +
  geom_line(color = "#2c4a6e", linewidth = 1) +
...
```{r ex6-6-plot, echo=T, warning=FALSE}
...
```{r ex6-7-plot, echo=T, warning=FALSE}
x <- seq(mu - 4*sigma, mu + 4*sigma, length.out = 300)
df <- data.frame(x = x, y = dnorm(x, mu, sigma))
...
```{r clt-demo, echo=T, warning=FALSE}
