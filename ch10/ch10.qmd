...
## Example 10-1: Car Rental Companies — Scatter Plot {#example-10-1}
...
## Example 10-2: Absences and Final Grades — Scatter Plot {#example-10-2}
...
## Example 10-3: Age and Wealth — Scatter Plot {#example-10-3}
...
## Example 10-4: Correlation for Car Rental Companies {#example-10-4}
...
## Example 10-5: Correlation for Absences and Final Grades
...
**Result:** r ≈ −0.944 → strong negative linear relationship.

---

## Example 10-6: Correlation for Age and Wealth {#example-10-6}
...
**Result:** r ≈ −0.176 → very weak relationship, likely due to chance.
...
## Example 10-7: Test Significance of r = 0.982
...
## Example 10-8: Test Significance Using Table I (r = −0.176)
...
d.f. = 8 − 2 = 6; From Table I at α = 0.01, d.f. = 6: critical value = **0.834**

|r| = 0.176 < 0.834 → **Do not reject H₀**

There is **not enough evidence** to conclude a significant linear relationship between age and wealth.

```{r echo=T, warning=FALSE}
x <- c(73, 65, 53, 54, 79, 69, 61, 65)
y <- c(16, 26, 50, 21.5, 40, 16, 19.6, 19)
test <- cor.test(x, y)
cat("r =", round(test$estimate, 4), "  P-value =", round(test$p.value, 4), "\n")
cat("Decision:", ifelse(test$p.value < 0.01, "Reject H0", "Do not reject H0"), "\n")
```
...
## Example 10-9: Regression Line for Car Rental Data {#example-10-9}
...
ggplot(data.frame(x, y), aes(x, y)) +
  ...
  labs(title = "Car Rental: Revenue vs Fleet Size",
       x = "Cars (×10,000)", y = "Revenue ($ billions)") +
  theme_minimal()
```

---

## Example 10-10: Regression Line for Absences and Grades
...
model2 <- lm(y ~ x)
cat("Intercept a =", round(coef(model2)[1], 3), "\n")
cat("Slope b =",     round(coef(model2)[2], 3), "\n")
...
## Example 10-11: Making a Prediction
...
## Example 10-12: Copy Machine Maintenance Costs {#example-10-12}
...
## Example 10-13: Standard Error via Computational Formula
...
## Example 10-14: 95% Prediction Interval
...
## Example 10-13: State Board Scores — Multiple Regression
...
## Example 10-15: Multiple Correlation Coefficient
...
## Example 10-16: F Test for Multiple Correlation
...
It is always ≤ R² and penalizes for adding unnecessary predictors.
:::
