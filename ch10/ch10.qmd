...
```{r e10-1-2, echo=T, warning=FALSE, out.width="80%", fig.align='center', fig.height=2.9}
p1
```
...
```{r e10-2-2, echo=T, warning=FALSE, out.width="80%", fig.align='center', fig.height=2.9}
p2
```
...
p3 <- ggplot(df3, aes(x = age, y = wealth)) +
...
```{r e10-3-2, echo=T, warning=FALSE, out.width="80%", fig.align='center', fig.height=2.9}
p3
```
...
Test whether the correlation r = 0.982 from [Example 10-4](#example-10-4) is significant at $\alpha$ = 0.05.
:::
...
**Step 2:** d.f. = 6 − 2 = 4; CV = $\pm$2.776 (from t-table)
...
Using Table I, test the significance of r = −0.176 from [Example 10-6](#example-10-6) at $\alpha$ = 0.01.
:::
...
```{r e10-9-1-fig, echo=T, warning=FALSE, out.width="80%", fig.align='center', fig.height=4.0}
...
```{r e10-10-1-fig, echo=T, warning=FALSE, out.width="80%", fig.align='center', fig.height=4.0}
ggplot(data.frame(x, y), aes(x, y)) +
  geom_point(size = 3, color = "tomato") +
  geom_smooth(method = "lm", se = FALSE, color = "darkred") +
  labs(title = "Absences vs. Final Grade",
       x = "Number of Absences", y = "Grade (%)") +
  theme_minimal()
```
:::
...
```{r e10-11-1, echo=T, warning=FALSE, out.width="80%", fig.align='center'}
x <- c(63.0, 29.0, 20.8, 19.1, 13.4, 8.5)
y <- c(7.0, 3.9, 2.1, 2.8, 1.4, 1.5)
model <- lm(y ~ x)
...
*(R uses the unrounded coefficients and gives 2.519; the hand value 2.516 uses a and b rounded to three decimals.)*
:::
...
```{r e10-11-2, echo=T, warning=FALSE, out.width="80%", fig.align='center'}
...
Using the shortcut formula and the data from [Example 10-12](#example-10-12) (where $\sum y^2$ = 42,186, $\sum y$ = 496, $\sum xy$ = 1778, a = 55.57, b = 8.13):
:::
...
R's `predict()` gives the interval (**60.36, 99.55**), slightly different from the hand calculation (60.53, 99.39) because the hand calculation uses rounded values of a, b, $s_{est}$, and the SE factor. We are 95% confident the actual maintenance cost for a 3-year-old machine falls in this interval.
:::
...
```{r e10-mr-1, echo=T, warning=FALSE, out.width="80%", fig.align='center'}
...
```{r e10-mr-2, echo=T, warning=FALSE, out.width="80%", fig.align='center'}
