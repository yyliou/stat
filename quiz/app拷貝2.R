...
# ── Configuration ─────────────────────────────────────────────
EXAM_TITLE <- "Elementary Statistics — Online Quiz"
...
.qz-header {
  ...
  padding: 16px 28px; border-radius: 8px 8px 0 0;
}
.qz-header h2 { margin: 0 0 3px; font-size: 1.3rem; }
.qz-header p  { margin: 0; opacity: .7; font-size: .83rem; }
.qz-body {
...
.q-text {
  ...
  padding:13px 17px; border-radius:0 6px 6px 0;
  margin-bottom:18px; font-size:1.03rem; line-height:1.65;
}
.lbl  { font-weight:600; font-size:.87rem; color:#444; margin-bottom:4px; }
.note { font-size:.79rem; color:#999; margin-bottom:6px; }
...
.inst-list li { margin-bottom:8px; line-height:1.6; }
.notice {
...
.notice.warn { background:#fff3cd; border-color:#f0ad4e; font-size:.92rem; }
.done-box  { text-align:center; padding:44px 20px; }
.done-icon { font-size:3.5rem; color:#28a745; }
.done-box h3 { margin:12px 0 6px; }
.done-box p  { color:#666; }
  ...
  div(class = "qz-wrap",
    ...
    div(class = "qz-body",
      ...
      div(id = "pg-login",
        ...
        fluidRow(
          column(5, textInput("inp_sid",  "Student ID:", placeholder = "e.g. B12345678")),
          column(5, textInput("inp_name", "Full Name:",  placeholder = "e.g. John Smith"))
        ),
        br(),
      ...
      lapply(seq_len(N), function(i) {
        div(id = paste0("pg-q", i), style = "display:none;",
          ...
          tags$p(style = "color:#888; font-size:.87rem; margin-bottom:5px;",
